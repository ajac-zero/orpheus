...
impl Orpheus {
    ...
    pub fn simple_chat_stream(
        ...
        self.chat().model(model).messages(vec![message]).stream()
    }
}
...
impl<S: chat_request_builder::State> ChatRequestBuilder<S> {
    pub fn send(mut self) -> crate::Result<ChatCompletion>
    where
    ...
    pub fn stream(mut self) -> crate::Result<ChatStream>
    where
        S: chat_request_builder::IsComplete,
    {
        let handler = self.handler.take().unwrap();
...
mod tests {
    ...
    fn test_chat_request_with_image() {
        ...
        let response = client
            ...
            .model("openai/gpt-4o")
            .messages(vec![
                ChatMessage::system(Content::simple("You are a photography connoseur.")),
                ChatMessage::user(Content::simple("What do you think of this image?").with_image("https://upload.wikimedia.org/wikipedia/commons/thumb/d/dd/Gfp-wisconsin-madison-the-nature-boardwalk.jpg/2560px-Gfp-wisconsin-madison-the-nature-boardwalk.jpg", None)),
            ])
            .send();
        println!("{:?}", response);
    ...
    #[test]
    fn test_chat_request_with_file() {
        let api_key = env::var(API_KEY_ENV_VAR).expect("load env var");
        ...
        let response = client
            ...
            .messages(vec![ChatMessage::user(
                Content::simple("Can you tell me the contents of this pdf?").with_file(
                    "dummy.pdf",
