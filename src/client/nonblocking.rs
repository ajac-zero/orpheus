...
impl AsyncOrpheus {
    ...
    pub fn chat(
        ...
        messages: impl Into<ChatMessages>,
    ) -> ChatRequestBuilder {
        let handler = self.clone();
        ChatRequest::builder(Some(handler), model, messages)
    }
}
...
mod tests {
    ...
    async fn test_chat_request() {
        ...
        let response = client
            .chat(
                ...
                [Message::system("You are a friend"), Message::user("Hello!")],
            )
            .send()
            .await;
        println!("{:?}", response);
    ...
    async fn test_chat_request_with_file() {
        ...
        let response = client
            .chat(
            "openai/gpt-4o",
            Message::user("Can you tell me the contents of this pdf?").with_file(
                    "dummy.pdf",
    ...
    async fn test_chat_request_with_tool() {
        ...
        let response = client
            .chat("google/gemini-2.0-flash-001", "Isabella is 12 years old.")
            .tools(Vec::from([tool]))
            .send()
            .await;
        println!("{:?}", response);
