...
class Runner:
    ...
    def evaluate(self):
        results = [self.time_function(func, name) for name, func in self.targets]
    ...
    def time_function(self, func, name):
...
if __name__ == "__main__":
    ...
    native_orpheus_func = functools.partial(
        orpheus.message, messages=NATIVE_MESSAGES, model=MODEL
    )

    console = Console()

    # Print header
    console.print("[bold]SDK Benchmark[/bold]", style="yellow")
    console.print(
    ...
    # Run benchmark
    Runner(console, targets, args.runs, args.variance, args.save).evaluate()
